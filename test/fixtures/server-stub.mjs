...
const seen=new Set();
const seven=['deviceId','hanaRef','action','occurredAt','eventId','nonce','signature'],optional=['sourceHanaRef','targetRef','receipt'];
const shapeOk=event=>{const keys=Object.keys(event);return seven.every(k=>keys.includes(k))&&keys.every(k=>seven.includes(k)||optional.includes(k));};
...
const server=createServer(async(request,response)=>{
  ...
  if(request.method==='POST'&&url.pathname==='/v1/usage/events'){
    ...
    for(const event of events){if(event===null||typeof event!=='object'||!shapeOk(event)){rejected.push({eventId:typeof event?.eventId==='string'?event.eventId:'invalid',code:'USAGE_INPUT_INVALID'});continue;}if(seen.has(event.eventId))duplicates++;else{seen.add(event.eventId);accepted++;}}
