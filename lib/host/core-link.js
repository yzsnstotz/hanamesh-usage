// @ts-check
import { signingJSON, UsageError, validateEvent, wireEvent } from '../core/index.js';
...
export function signUsageEvent(core,input) {
  ...
  // O1 verifies ed25519 over JSON.stringify({deviceId,hanaRef,action,occurredAt,eventId,nonce}) in that key order (not sorted keys).
  const signature=core.sign(new TextEncoder().encode(signingJSON(six)));
