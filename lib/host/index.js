...
import { z } from 'zod';
import { UsageError, isSnapshot } from '../core/index.js';
...
export async function apply(ctx, config = {}) {
  ...
  const registry = typeof ctx.agents?.readBinding === 'function' ? ctx.agents : null;
  await ctx.effect(async () => {
    const domain = await ctx.storage.domain.open(usageDomainSpec);
    try {
      const mounted = mountUsage(ctx, registry, domain, resolved);
      await mounted.ready;
