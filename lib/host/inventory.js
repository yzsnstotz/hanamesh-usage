...
export function createInventory({ctx,store,link,inventoryIntervalMs,inspector=inspectPackage,now=Date.now,note=()=>{}}){
  ...
  async function scanRaw(){
    ...
    const core=link.get(),inputs=[],byId=new Map();
    if(core!==null&&core.getConsent()==='granted'){
      const deviceId=core.getDeviceId(),existingIds=new Set(store.getSnapshot().events.map(event=>event.eventId));
      ...
      for(const item of items){const eventId=eventIdForLoader(deviceId,'install',item.hanaRef,item.version);if(!existingIds.has(eventId)&&!changes.some(change=>change.action==='install'&&change.hanaRef===item.hanaRef&&change.version===item.version))changes.push({action:/** @type {const} */('install'),hanaRef:item.hanaRef,version:item.version});}
    ...
    const committed=await store.updateInventory(scannedAt,items,inputs);lastScanAt=scannedAt;return {scannedAt,events:committed.inserted.map(id=>byId.get(id)),unreadable};
  }
