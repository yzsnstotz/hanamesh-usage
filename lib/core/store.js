...
export class ActivityStore {
    ...
    constructor(global, maxRecords = 5000) {
        ...
        if (!Number.isSafeInteger(maxRecords) || maxRecords < 1 || maxRecords > 100000)
            throw new ActivityError('INVALID_CAPACITY');
        validateSnapshot(global.get());
    }
    put(value) {
        if (this.closed)
            return Promise.reject(new ActivityError('STORE_CLOSED'));
    ...
    query(filter = {}) {
        if (this.closed)
            throw new ActivityError('STORE_CLOSED');
        const snapshot = this.global.get();
        validateSnapshot(snapshot);
