...
export function validTargetRef(value) { return typeof value === 'string' && targetToken.test(value) && !secretLike.test(value); }
export function validReceipt(value) {
    if (!isObject(value) || !exact(value, ['providerId', 'model', 'count']))
        return false;
    if (typeof value.providerId !== 'string' || !providerToken.test(value.providerId) || secretLike.test(value.providerId))
        return false;
    if (value.model !== null && (typeof value.model !== 'string' || !modelToken.test(value.model) || secretLike.test(value.model)))
        return false;
...
export function validateEvent(input) {
    if (!isObject(input) || !exactWithOptional(input, REQUIRED_KEYS, OPTIONAL_KEYS))
        invalid();
    ...
    if (input.signature !== null && (typeof input.signature !== 'string' || !base64url.test(input.signature)))
        invalid();
    if (input.sourceHanaRef !== null && !validHanaRef(input.sourceHanaRef))
        invalid();
    if (input.targetRef !== null && !validTargetRef(input.targetRef))
        invalid();
    if (Object.hasOwn(input, 'receipt') && input.receipt !== null && (input.action !== 'use' || !validReceipt(input.receipt)))
        invalid();
    if (!['session-log', 'loader', 'seat'].includes(String(input.source)))
        invalid();
...
export function createUsageEvent(input) {
    ...
    const event = { schemaVersion: 1, ...rest, sourceHanaRef, targetRef, receipt, upload: { state: 'pending', code: null, attempts: 0, sentAt: null } };
    validateEvent(event);
