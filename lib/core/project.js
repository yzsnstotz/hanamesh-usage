...
export function eventReference(sessionId, turn) {
    ...
    if (!Number.isSafeInteger(turn) || turn < 0)
        throw new UsageError('INVALID_TURN');
...
function tokenUsage(events, startSeq, endSeq, turn) {
    ...
    if (payloads.length === 0)
        return { inputTokens: missing('usage_not_reported'), outputTokens: missing('usage_not_reported'), totalTokens: missing('usage_not_reported') };
    const fields = ['inputTokens', 'outputTokens', 'totalTokens'];
    const sum = (field) => {
        let total = 0;
        for (const payload of payloads) {
            const value = object(payload)[field];
            if (field === 'totalTokens' && value === undefined)
                return missing('not_provided');
...
export function projectTerminal(session, end) {
    ...
    if (end.type !== 'turn/end' || !Number.isSafeInteger(end.seq) || end.seq < session.inheritedEventCount)
        throw new UsageError('NOT_OWN_TERMINAL');
    ...
    if (binding && binding.sessionId !== sid)
        throw new UsageError('BINDING_ID_MISMATCH');
