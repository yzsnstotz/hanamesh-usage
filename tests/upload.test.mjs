...
test('U13 accepted, duplicate and rejected outcomes become terminal in one batch',async()=>{
  ...
  const result=await f.reporter.runOnce();assert.equal(result,'uploaded');assert.equal(f.requests.length,1);const body=JSON.parse(f.requests[0].options.body);assert.equal(body.events.length,3);assert(body.events.every(event=>Object.keys(event).length===7));assert.equal(f.requests[0].options.headers.origin,'https://usage.example');
