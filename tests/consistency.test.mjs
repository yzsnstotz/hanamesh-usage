...
test('X01 SOURCE: declaration schema valid and all stateful groups/boundaries enumerated',()=>{
  validateConsistency(c);assert.equal(c.exempt,false);assert.equal(c.groups.length,2);assert.equal(c.boundaries.length,3);assert.deepEqual(c.boundaries.map(item=>item.safeDirection),['storage-domain:hanamesh_usage','remote:srv_usage_events','storage-domain:hanamesh_usage_events']);
});
