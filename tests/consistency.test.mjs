...
test('X01 SOURCE: declaration schema valid and all stateful groups/boundaries enumerated',()=>{
  validateConsistency(c);assert.equal(c.exempt,false);assert.equal(c.groups.length,1);assert.equal(c.boundaries.length,1);assert.equal(c.boundaries[0].safeDirection,'dsh:session-log');
});
