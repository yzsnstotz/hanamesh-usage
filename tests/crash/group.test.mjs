...
for(const [point,count] of [['before-rename',0],['after-rename',1]])test(`X02 FIXTURE POSIX: actual SIGKILL ${point}, restart sees whole image or none`,async t=>{
  const dir=await mkdtemp(join(tmpdir(),'activity-x02-'));try{
