...
function fixture({allowExport=false,fail=false}={}){
  const routes=new Map(),effects=[],api={query:filter=>{if(fail)throw Error('sk-SYNTHETIC_SECRET');return core.queryRecords([record()],filter);},export:filter=>{if(!allowExport)throw new core.UsageError('EXPORT_DISABLED');return core.queryRecords([record()],filter);},health:()=>({pending:0,recoveryComplete:true,failures:{}})};
...
test('query API uses no-store headers; HTML has distinct unavailable label',async()=>{
  ...
  const view=await f.request('/api/hanamesh/usage/view');assert.match(view.headers.get('content-security-policy'),/default-src 'none'/);assert.match(await view.text(),/— unavailable/);await f.close();
});
...
test('transport unexpected errors never serialize original Error',async()=>{
  const f=fixture({fail:true}),r=await f.request('/api/hanamesh/usage');assert.equal(r.status,503);assert.deepEqual(await r.json(),{error:'ACTIVITY_UNAVAILABLE'});await f.close();
});
