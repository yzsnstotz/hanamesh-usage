...
for(const item of cases){
 const dir=mkdtempSync(join(tmpdir(),'activity-mutant-'));
 try{
  const command=['--test','--test-reporter=tap',...(item.pattern?[`--test-name-pattern=${item.pattern}`]:[]),item.test];
  const baseline=spawnSync(process.execPath,command,{encoding:'utf8',env:{...process.env,ACTIVITY_TEST_ROOT:root}});
  ...
  const mutant=spawnSync(process.execPath,command,{encoding:'utf8',env:{...process.env,ACTIVITY_TEST_ROOT:dir}});
