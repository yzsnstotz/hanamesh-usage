import {spawnSync} from 'node:child_process';import {readFileSync,existsSync} from 'node:fs';import {homedir} from 'node:os';import {resolve} from 'node:path';import {fileURLToPath} from 'node:url';import {installedPeer} from './pins.mjs';
...
for(const [name,version] of Object.entries(pkg.peerDependencies)){let actual=null;try{actual=installedPeer(name).version;}catch{}checks[name]={target:version,actual};}
...
if(Object.values(checks).some(c=>c.actual!==c.target)||!isolatedHome){console.error('PRECONDITIONS_NOT_MET: pinned host/toolchain or isolated DSH_HOME unavailable; no host tests attempted.');process.exitCode=2;}
