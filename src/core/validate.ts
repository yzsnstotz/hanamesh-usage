...
function obs(v: unknown, kind: 'text' | 'number' | 'result'): void {
  assert(isObject(v));
  if (v.state === 'unavailable') {
    keys(v, ['state','value','reason']); assert(v.value === null && reasons.includes(String(v.reason))); return;
  }
  ...
  if (kind === 'number') assert(typeof v.value === 'number' && Number.isSafeInteger(v.value) && v.value >= 0);
  else if (kind === 'result') assert(['completed','failed','interrupted'].includes(String(v.value)) && v.state === 'reported');
  else assert(safeMetadata(v.value));
}
export function validateDeclaration(v: unknown): asserts v is Declaration {
  ...
  keys(v.executor,['id','version','protocolVersion']); for (const item of Object.values(v.executor)) obs(item,'text');
  keys(v.lineage,['parentSession','parentExecution','rootExecution']); for (const item of Object.values(v.lineage)) obs(item,'text');
  obs(v.provider,'text'); obs(v.model,'text'); obs(v.result,'result'); obs(v.error,'text');
  assert(['completed','error','aborted','interrupted','blocked','max-tokens','unsupported'].includes(String(v.terminalReason)));
  const result = v.result as Declaration['result'];
  ...
  const error = v.error as Declaration['error'];
  assert(error.state === 'unavailable' || (v.terminalReason === 'error' && error.value === 'runtime_error_details_withheld'));
  keys(v.time,['startedAt','endedAt']); obs(v.time.startedAt,'number'); obs(v.time.endedAt,'number');
  const time = v.time as unknown as Declaration['time'];
...
export function validateSnapshot(value: unknown): asserts value is Snapshot {
  keys(value, ['schemaVersion','records']); assert(value.schemaVersion === 1 && Array.isArray(value.records));
  const identities = new Set<string>();
  for (const record of value.records as unknown[]) {
    validateDeclaration(record); assert(!identities.has(record.eventRef)); identities.add(record.eventRef);
  }
}
export function isSnapshot(value: unknown): value is Snapshot {
  try { validateSnapshot(value); return true; } catch { return false; }
}
