...
export type ValueSource = 'registry.binding' | 'session.header' | 'session.turn/start' | 'session.turn/end'
  | 'session.request/context' | 'session.assistant/message' | 'synthetic.fixture';
export type MissingReason = 'not_provided' | 'redacted' | 'binding_unavailable'
  | 'execution_lineage_not_exposed' | 'not_applicable' | 'route_not_observed'
  | 'multiple_routes' | 'usage_not_reported' | 'usage_contract_missing'
  | 'unsupported_terminal_reason' | 'invalid_value';
export type Observation<T> =
  | { state: 'reported' | 'estimated'; value: T; source: ValueSource; rule: string }
  | { state: 'unavailable'; value: null; reason: MissingReason };
...
export interface Declaration {
  ...
  eventRef: string;
  execution: { sessionId: string; turn: number };
  executor: { id: Observation<string>; version: Observation<string>; protocolVersion: Observation<string> };
  lineage: { parentSession: Observation<string>; parentExecution: Observation<string>; rootExecution: Observation<string> };
  provider: Observation<string>;
  model: Observation<string>;
  time: { startedAt: Observation<number>; endedAt: Observation<number> };
...
export interface Snapshot { schemaVersion: 1; records: Declaration[] }
...
export interface EventObservation { type: string; seq: number; time: number; data: unknown }
export interface BindingObservation {
  sessionId: string; runtime: string; driverVersion: string; protocolVersion?: string;
}
...
export interface Bucket { sum: number | null; count: number }
export interface MetricTotal { reported: Bucket; estimated: Bucket; unavailable: number }
export interface QueryResult {
  total: number;
  records: Declaration[];
  aggregate: { inputTokens: MetricTotal; outputTokens: MetricTotal; totalTokens: MetricTotal };
}
