...
export function validateEvent(input: unknown): asserts input is UsageEvent {
  if (!isObject(input) || !exact(input, ['schemaVersion','eventId','deviceId','hanaRef','action','occurredAt','nonce','signature','sourceHanaRef','targetRef','source','sourcePlugin','evidenceRef','upload'])) invalid();
  ...
  if (input.sourceHanaRef !== null || input.targetRef !== null || !['session-log','loader','seat'].includes(String(input.source))) invalid();
...
export function createUsageEvent(input: Omit<UsageEvent,'schemaVersion'|'sourceHanaRef'|'targetRef'|'upload'>): UsageEvent {
  const event: UsageEvent = { schemaVersion:1, ...input, sourceHanaRef:null, targetRef:null, upload:{state:'pending',code:null,attempts:0,sentAt:null} };
  validateEvent(event);
